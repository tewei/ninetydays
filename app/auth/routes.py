...
def login():
    ...
    if form.validate_on_submit():
        user = UserBasic.query.filter_by(username=form.username.data).first()
...
def register():
    ...
    if form.validate_on_submit():
        user = UserBasic(username=form.username.data, email=form.email.data)
...
def reset_password(token):
    if current_user.is_authenticated:
        return redirect(url_for('main.index'))
    user = UserBasic.verify_reset_password_token(token)
    if not user:
        return redirect(url_for('main.index'))
